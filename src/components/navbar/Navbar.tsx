...
export const Navbar: React.FC<NavbarProps> = ({ currentPath, onNavigate, isDark, setIsDark }) => {
  ...
  return (
    ...
    >
          ...
          {navConfig.map((item) => (
            ...
            >
              <button 
                ...
                className={`relative flex items-center gap-2 px-6 py-2 rounded text-base font-medium transition-all duration-300 cursor-pointer ${
                  currentPath === item.path || activeMenu === item.title
                  ? 'text-foreground' 
      ...
      {/* Mega Menu Overlay */}
      <div 
        className={`absolute left-0 right-0 top-full bg-background border-b border-foreground/10 shadow-md transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] overflow-hidden ${
          activeMenu ? 'max-h-[60vh] opacity-100' : 'max-h-0 opacity-0 pointer-events-none'
        }`}
      >
          ...
          <div className="grid grid-cols-12 gap-16">
            <div className="col-span-8 grid grid-cols-2 gap-x-12 gap-y-10">
              {activeMenu && navConfig.find(n => n.title === activeMenu)?.subItems?.map((sub, idx) => (
                ...
                >
                  <div className="w-12 h-12 rounded bg-slate-50 dark:bg-slate-800 flex items-center justify-center shrink-0 group-hover:bg-emerald-500 group-hover:text-white transition-all duration-500 group-hover:scale-110 group-hover:rotate-3 shadow-sm group-hover:shadow-2xl group-hover:shadow-emerald-500/40">
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d={sub.icon} /></svg>
                  </div>
                  <div>
                    <div className="text-sm font-black uppercase tracking-widest text-slate-900 dark:text-white mb-2 group-hover:text-emerald-500 transition-colors">
                      {sub.title}
                    </div>
                    <div className="text-xs font-bold text-slate-400 dark:text-slate-500 leading-relaxed">
                      {sub.desc}
                    </div>
                  </div>
                </button>
            ...
            <div className="col-span-4 pl-16 border-l border-slate-100 dark:border-slate-800">
              <div className="p-10 rounded bg-slate-50 dark:bg-slate-800/50 space-y-8 relative overflow-hidden group/card">
                <div className="space-y-4">
                  <span className="inline-block px-4 py-1 rounded-full bg-emerald-500/10 text-emerald-500 text-[10px] font-black uppercase tracking-widest">Support</span>
                  <h4 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight leading-none uppercase">Нужна помощь?</h4>
                  <p className="text-xs font-bold text-slate-400 dark:text-slate-500 leading-relaxed uppercase tracking-wider">Ответим на любой вопрос за 5 минут.</p>
                </div>
                <a href="tel:88005553322" className="flex items-center gap-4 text-slate-900 dark:text-white group/link">
                  <div className="w-12 h-12 rounded bg-white dark:bg-slate-700 flex items-center justify-center shadow-lg group-hover/link:bg-emerald-500 group-hover/link:text-white transition-colors">
